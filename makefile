...
fire: pull
	...
	echo "[phantom-1]"                                                                            >> autogenerated.ini
	echo "headers = [Host: icat-test04]"                                                          >> autogenerated.ini
	echo "address = icat-test04:${GATEPORT}"                                                      >> autogenerated.ini
	echo "uris = /kload${EVENTSIZE}"                                                              >> autogenerated.ini
	echo "rps_schedule = line(1, ${MAXRPS}, 60s) const(${MAXRPS}, ${TESTDUR}s)"                   >> autogenerated.ini
	echo "[phantom-2]"                                                                            >> autogenerated.ini
	echo "headers = [Host: icat-test05]"                                                          >> autogenerated.ini
	echo "address = icat-test05:${GATEPORT}"                                                      >> autogenerated.ini
	echo "uris = /kload${EVENTSIZE}"                                                              >> autogenerated.ini
	echo "rps_schedule = line(1, ${MAXRPS}, 60s) const(${MAXRPS}, ${TESTDUR}s)"                   >> autogenerated.ini
	...
	for host in edi18 icat-test04 icat-test05 ; do \
		...
		echo "    <Custom diff=\"1\" measure=\"call\" label=\"mean-travel-time\">curl -s http://$$host:8889/mtt</Custom>" >> monitoring.xml ; \
		echo "  </Host>"                                                                          >> monitoring.xml ; \
	done
	echo "</Monitoring>"                                                                          >> monitoring.xml
